...
import { Tag, Grid, Car, Flower, Table } from "lucide-react";

const ICONS: Record<string, JSX.Element> = {
  "Ghế đá": <Car className="w-5 h-5 mr-1" />,
  "Bàn đá": <Table className="w-5 h-5 mr-1" />,
  "Chậu cảnh": <Flower className="w-5 h-5 mr-1" />,
};
  ...
  return (
    <div className="flex flex-wrap gap-2 justify-center mb-8">
      ...
      {categories.map((cat) => (
        ...
        >
          {ICONS[cat] || <Tag className="w-5 h-5 mr-1" />} {cat}
        </button>
      ))}
    </div>
  );
